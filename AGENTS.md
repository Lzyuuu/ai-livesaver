...
仓库根目录下的 `V4.51-github-release.apk` 是 **fancy-ai** 的安装包，它是本项目的参考 APP。开发时需要参考它的设计与整体 APP 功能进行复刻：UI 布局、交互流程、页面结构与核心功能都应以该 APK 的实际表现为准。可使用 `mobile-mcp` 将其安装到模拟器/真机上进行截图、录屏和页面流程走查，作为实现与验收的对照依据。
