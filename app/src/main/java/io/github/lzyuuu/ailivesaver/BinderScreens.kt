...
internal fun BinderScreen(contentPadding: PaddingValues, store: WorldStore, onBack: () -> Unit, onChanged: () -> Unit, onOpenMessenger: (Long) -> Unit) {
    ...
    @Composable fun field(key: String, label: String, value: String, update: (String)->Unit) { OutlinedTextField(value, update, label={Text(label)}, modifier=Modifier.fillMaxWidth().testTag("binder-field-$key"), minLines=2, colors=OutlinedTextFieldDefaults.colors(focusedTextColor=FancyCream,unfocusedTextColor=FancyCream,focusedLabelColor=FancyGold,unfocusedLabelColor=FancyCream)) }
    fun generate() { busy=true; error=null; BinderOrchestrator.generate(context,draftId,answers) { result -> result.onSuccess { generation -> candidates=generation.candidates; step=5 }.onFailure { error="Provider 失败：${it.message}" }; busy=false } }
    Column(Modifier.fillMaxSize().background(FancyInk).padding(contentPadding).padding(20.dp).verticalScroll(rememberScrollState()), verticalArrangement=Arrangement.spacedBy(12.dp)) {
        ...
        if(step<4) { val labels=listOf("关系与偏好" to listOf("relationship" to "期待的关系", "preferences" to "相处偏好"),"人格与沟通" to listOf("personality" to "人格气质", "communication" to "沟通方式"),"兴趣" to listOf("interests" to "兴趣与日常"),"边界与排除" to listOf("boundaries" to "边界", "exclusions" to "明确排除"))[step]; Text(labels.first,color=FancyCream); labels.second.forEach { (key,label) -> val value=when(key){"relationship"->answers.relationship;"preferences"->answers.preferences;"personality"->answers.personality;"communication"->answers.communication;"interests"->answers.interests;"boundaries"->answers.boundaries;else->answers.exclusions}; field(key,label,value){ v->answers=when(key){"relationship"->answers.copy(relationship=v);"preferences"->answers.copy(preferences=v);"personality"->answers.copy(personality=v);"communication"->answers.copy(communication=v);"interests"->answers.copy(interests=v);"boundaries"->answers.copy(boundaries=v);else->answers.copy(exclusions=v)}; store.putBinderDraft(BinderDraft(draftId,step,answers.toJson(),System.currentTimeMillis())) } }; Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.SpaceBetween){if(step > 0) TextButton(onClick = { save(step-1) }){Text("上一步",color=FancyGold)};Button({if(answers.relationship.isBlank()&&step==0) error="请填写关系偏好" else save(step+1)},modifier=Modifier.testTag("binder-next")){Text("下一步")}} } else if(step==4) { Text("Review",color=FancyCream); Text(answers.toJson(),color=FancyCream); Button({generate()},enabled=!busy,modifier=Modifier.testTag("binder-generate")){Text(if(busy)"生成中…" else "生成候选")} } else { Text("选择匹配",color=FancyCream); candidates.forEachIndexed { i,c -> Card(Modifier.fillMaxWidth().testTag("binder-candidate-$i")){Column(Modifier.padding(14.dp)){Text(c.name,color=FancyCream);Text(c.persona,color=FancyCream);Text(c.reasons.joinToString(" · "),color=FancyGold);Button({val id=BinderOrchestrator.confirm(context,draftId,i,c); onChanged();onOpenMessenger(id)}){Text("确认并聊天")}}} }; TextButton({save(4)}){Text("重新生成",color=FancyGold)} }; error?.let{Text(it,color=FancyCream,modifier=Modifier.testTag("binder-error"))}
    }
}
